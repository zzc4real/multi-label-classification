...
def load_test_data(test_file, labels):
    ...
    select = ['Caption','ImageID']

    df = df.dropna(axis=0, how='any', subset=select)
    # test_examples = df[select[0]].tolist()
...
def predict_unseen_data(result, target):
    trained_dir = result
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            df['Labels'] = predict_labels
            # columns = sorted(df.columns)
            df.to_csv(predicted_dir + 'predictions_all.csv', index=False,  sep=',')
...
if __name__ == '__main__':
    train_result = './trained_results_1590475107'
    target = './data/test.csv'

    predict_unseen_data(train_result, target)
